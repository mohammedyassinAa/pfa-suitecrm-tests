...
export class ImportLeadPage {
    ...
    // ========== UNHAPPY PATH METHODS ==========

    async uploadMalformedCSV(filePath: string) {
      await this.objects.fileInput.setInputFiles(filePath);
      this.objects.page.once('dialog', async dialog => {
        await dialog.accept();
      });
    ...
    async uploadCSVWithInvalidEmail(filePath: string) {
      await this.objects.fileInput.setInputFiles(filePath);
      this.objects.page.once('dialog', async dialog => {
        await dialog.accept();
      });
    ...
    async verifyImportErrorMessage(expectedMessage: string) {
      await expect(this.objects.errMsg).toBeVisible(); 
      await expect(this.objects.errMsg).toContainText(expectedMessage);
    }
}
